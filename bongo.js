...
bongo.on("ready", () => {
  ...
  bongo.on("error", (e) => console.error(e));
  bongo.on("warn", (e) => console.warn(e));
  bongo.on("debug", (e) => console.info(e));
...
fs.readdir("./commands/", (err, files) => {
  ...
  files.forEach(file => {
    ...
    bongo.commands.set(commandName, props);
  });
});
...
const init = async () => {
  ...
  bongo.logger.log(`Loading a total of ${cmdFiles.length} commands.`);
  cmdFiles.forEach(f => {
    if (!f.endsWith(".js")) return;
    const response = bongo.loadCommand(f);
    if (response) console.log(response);
  });

  // Then we load events, which will include our message and ready event.
  const evtFiles = await readdir("./events/");
  bongo.logger.log(`Loading a total of ${evtFiles.length} events.`);
  evtFiles.forEach(file => {
    const eventName = file.split(".")[0];
    bongo.logger.log(`Loading Event: ${eventName}`);
    const event = require(`./events/${file}`);
