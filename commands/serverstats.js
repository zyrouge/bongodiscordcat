...
exports.run = async (bongo, message, args, tools) => {
...
if (!args[0]) return message.channel.send(":x: Invalid parameters. Correct usage: `b!serverstats enable` | `b!serverstats disable`.");  
if(args[0] === 'enable') {
...
if(totusers === null || membcount === null || botcount === null) return message.channel.send(`:x: Serverstats for this server is not enabled.`)
  bongo.channels.get(totusers).delete()
  bongo.channels.get(membcount).delete()
  bongo.channels.get(botcount).delete()
  bongo.channels.get(categ).delete()
